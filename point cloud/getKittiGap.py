...
GroundOutDir='./ground/'
# PoseBaseDir='../../data_odometry_poses/'
PoseBaseDir='/media/qzj/Document/grow/slam/slamDataSet/kitti/data_odometry_poses/dataset/poses/'

OutDir='./kitti_4096/'

# if not os.path.exists(RmgroundOutDir):
# 	os.mkdir(RmgroundOutDir)
# if not os.path.exists(GroundOutDir):
# 	os.mkdir(GroundOutDir)
...
def generateRmGroud(seq):
	...
	# 看一下这些是否有必要改成直接写字符串
	PointCloudDir=PCBaseDir+str(seq).zfill(2)+'/velodyne/'
	bin_num=len(os.listdir(PointCloudDir))
	RmgroundDir=RmgroundOutDir+str(seq).zfill(2)+'/'
	GroundDir=GroundOutDir+str(seq).zfill(2)+'/'

	if not os.path.exists(GroundDir):
		os.mkdir(GroundDir)
	if not os.path.exists(RmgroundDir):
		os.mkdir(RmgroundDir)
	for frame in range(bin_num):
		...
		pc_rmground=pc_rmground.reshape([-1,4])
		pc_ground=pc_ground.reshape([-1,4])
		if frame%1==0:
...
def LoadData(FrameGap,npoints,seq):
	print('start run seq'+str(seq))
	if isRmGround:
		PointCloudDir=PCBaseDir+str(seq).zfill(2)+'/'
	else:
		PointCloudDir=PCBaseDir+str(seq).zfill(2)+'/velodyne/'
	bin_num=len(os.listdir(PointCloudDir))
	SeqPose=np.loadtxt(PoseBaseDir+str(seq).zfill(2)+'.txt')
	...
	for frame in range(51,bin_num-FrameGap):
...
if __name__ == '__main__':
	...
	for seq in range(4,5):
		all_p.append(multiproc.Process(target=LoadData,args=(20,4096,seq)))
