...
def placeholder_inputs(batch_num_queries, num_pointclouds_per_query, num_point):
    pointclouds_pl = tf.compat.v1.placeholder(tf.float32, shape=(batch_num_queries, num_pointclouds_per_query, num_point, 13))
    return pointclouds_pl

#Adopted from the original pointnet code
def forward(point_cloud, is_training=tf.constant(False,dtype=tf.bool), bn_decay=None):
    ...
    with tf.variable_scope('SNfeature') as sc:
        ...
        idx_sn = nn_idx+idx_
        edge_feature = tf.gather(feature_cloud, idx_sn)
