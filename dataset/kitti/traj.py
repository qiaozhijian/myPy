...
eval_name = "KITTI_00_ORB.txt"
eval_name_new = "eval_name_new.txt"
gt_file = "KITTI_00_gt.txt"
...
for cnt, line in enumerate(s):
    ...
    for i in range(3):
        for j in range(4):
            T[i,j] = line_split[4*i + j]
    traj.append(T)
traj = np.asarray(traj).reshape((-1, 4, 4))
trajNew=np.zeros(traj.shape)
...
os.system("evo_traj kitti " + eval_name + " --ref "+ gt_file + " -p ")
# os.system("evo_ape tum "+ gt_file_new+ " " + eval_name + " --plot")
